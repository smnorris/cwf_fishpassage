...
WHERE downstream_barrier_id_15 IS NULL
GROUP BY watershed_group_code, downstream_barrier_id_structure),
...
(SELECT
  ...
  st_length(ST_Union(geom)) as length_segment
FROM cwf.segmented_streams
WHERE downstream_barrier_id_20 IS NULL
GROUP BY watershed_group_code, downstream_barrier_id_structure),
...
(SELECT
  ...
  st_length(ST_Union(geom)) as length_segment
FROM cwf.segmented_streams
WHERE downstream_barrier_id_30 IS NULL
GROUP BY watershed_group_code, downstream_barrier_id_structure),
