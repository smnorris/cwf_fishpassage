...
SELECT
  ...
  round((SUM(ST_Length(s.geom)) FILTER (WHERE s.downstream_barrier_id_15 IS NULL AND wb.waterbody_type IS NULL AND s.edge_type IN (1000,1050,1100,2000,2300) AND s.local_watershed_code IS NOT NULL) / 1000)::numeric, 1) AS stream_km_accessible15
 -- round((SUM(ST_Length(s.geom)) FILTER (WHERE s.downstream_barrier_id_15 IS NULL AND wb.waterbody_type = 'R' AND s.local_watershed_code IS NOT NULL) / 1000)::numeric,1) AS river_km,
...
SELECT
linear.*,
areal.lake_area_ha_total,
areal.lake_area_ha_accessible15,
areal.wetland_area_ha_total,
areal.wetland_area_ha_accessible15,
areal.reservoir_area_ha_total,
areal.reservoir_area_ha_accessible_15
from linear
inner join areal
